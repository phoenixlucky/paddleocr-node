        ...
        using (var img = new Bitmap(args[0]))
        {
            ...
            using (var small = new Bitmap(w, h, PixelFormat.Format24bppRgb))
            {
                ...
                try
                {
                    ...
                    double white = 0, edgeWhite = 0, edgePixels = 0, dark = 0, black = 0, colored = 0, transitions = 0, outerBlack = 0, outerPixels = 0;
                    for (var y = 0; y < h; y += 2)
                    {
                        var prevInk = false;
                        var row = y * stride;
                    ...
                    double confidence;
                    if (edgeWhiteRatio > .65 && blackRatio > .0035 && outerBlackRatio > .0025)
                    {
                        category = "dimension";
                        confidence = Math.Min(.95, .58 + blackRatio * 20 + outerBlackRatio * 30 + transitionRatio);
                        reason = "dimension_marks";
                    }
                    else if (edgeWhiteRatio > .82 && whiteRatio > .45 && darkRatio < .16 && outerBlackRatio < .0025)
                    {
